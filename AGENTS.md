...
- **验证**:`gestures.ts` 纯函数冒烟(esbuild 转译后 node 跑 24 断言:8 方向量化/直线/L 形/抖动合并/短行程/匹配/冲突/解析)。
