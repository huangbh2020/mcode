...
- **权限模式(Codex 自有一套,不套 Claude)**:`codex-readonly`(read-only+on-request)/`codex-workspace`(workspace-write+on-request,默认)/`codex-full`(danger-full-access+on-request)/`codex-bypass`(danger-full-access+never)。OS 沙箱(Seatbelt/Landlock)是主防线;审批只在动作要逃逸沙箱时由 server 发起,经 `ctx.requestApproval` 走既有审批卡。思考级别 minimal/low/medium/high/xhigh(Codex 原生)。
