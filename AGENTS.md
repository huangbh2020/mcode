...
- **`@anthropic-ai/claude-agent-sdk` 钉死精确版本 `0.3.218`(不带 `^`)**:防止 `^0.3.x` 在普通 `pnpm install` 时静默漂移(2026-08-23 曾漂到 0.3.238,捆绑 CLI 2.1.238 的 `sdkCompat.testedWrapperVersions` 甚至不含 wrapper 自身版本)。升级要显式改版本号,升级前必查:changelog + issues 搜 "Stream closed"/permission;新包 manifest 的 testedWrapperVersions 要含 wrapper 自身;`grep -ac "permission_exit_plan_mode_v2" claude.exe` 确认对话框 kind 没改名;升级后回归计划审批/AskUserQuestion/工具审批/子代理收尾四条链路
