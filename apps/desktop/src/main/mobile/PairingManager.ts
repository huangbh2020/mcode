...
export class PairingManager {
  ...
  /** Begin a pairing session. Idempotent-replacing: a second call supersedes
   *  any prior pending nonce. The endpoint comes from the HTTP server (it knows
   *  its bound port + detected LAN IP). */
  ...
  async verify(
    ...
    if (!safeEqualString(input.nonce, pending.nonce)) {
      ...
      return { ok: false, reason: "配对链接不匹配，请重新扫描 PC 端的最新二维码" };
    }
