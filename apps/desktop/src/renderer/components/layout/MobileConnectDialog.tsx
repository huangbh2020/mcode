 ...
 *   close → mobile.cancelPairing() (clears the nonce)
 */
...
import { IconDeviceMobile, IconRefresh, IconTrash } from "@renderer/lib/icons.js";
...
function MobileConnectPanel({ open }: { open: boolean }) {
  ...
  // cancelling on close broke the common test flow (open dialog → read code →
  // switch to the phone/browser to type it → pending already cancelled →
  // "invalid"). Closing the dialog just stops polling + clears local UI state.
  useEffect(() => {
    if (!open) {
      setPairing(null);
      setQrDataUrl(null);
  ...
  return (
    <>
      ...
      <Dialog.Description>用手机扫码并在 App 内输入验证码，完成配对（同一局域网）。</Dialog.Description>
      <Dialog.Close />

      {serverDown && (
        <div className="mt-3 rounded border border-yellow-500/40 bg-yellow-500/10 px-3 py-2 text-xs text-yellow-700 dark:text-yellow-300">
            ...
            {devices.map((d) => (
              ...
              >
                ...
                <button
                  ...
                  className="rounded p-1 text-content-subtle hover:bg-surface-hover hover:text-red-500"
                  title="断开该设备"
                >
                  <IconTrash size={14} />
                </button>
